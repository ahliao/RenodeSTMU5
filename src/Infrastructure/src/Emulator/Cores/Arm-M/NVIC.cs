    ...
    public class NVIC : IDoubleWordPeripheral, IHasDivisibleFrequency, IKnownSize, IIRQController, IHasMappedRegisters
    {
        ...
        public void WriteDoubleWord(long offset, uint value, bool isSecure)
        {
            ...
            case Registers.SystemHandlerPriority1:
                ...
                priorities[(int)(isSecure ? SystemException.MemManageFault_S : SystemException.MemManageFault)] = MaskPriority(value);
                priorities[(int)SystemException.BusFault] = MaskPriority(value >> 8);
                priorities[(int)(isSecure ? SystemException.UsageFault_S : SystemException.UsageFault)] = MaskPriority(value >> 16);
                this.DebugLog("Priority of IRQs 4, 5, 6 set to 0x{0:X}, 0x{1:X}, 0x{2:X} respectively.", MaskPriority(value), MaskPriority(value >> 8), MaskPriority(value >> 16));
                break;
            case Registers.SystemHandlerPriority2:
                // only 11th is not ignored
                priorities[(int)(isSecure ? SystemException.SuperVisorCall_S : SystemException.SuperVisorCall)] = MaskPriority(value >> 24);
                this.DebugLog("Priority of IRQ 11 set to 0x{0:X}.", MaskPriority(value >> 24));
                break;
            case Registers.SystemHandlerPriority3:
                priorities[(int)(isSecure ? SystemException.PendSV_S : SystemException.PendSV)] = MaskPriority(value >> 16);
                priorities[(int)(isSecure ? SystemException.SysTick_S : SystemException.SysTick)] = MaskPriority(value >> 24);
                this.DebugLog("Priority of IRQs 14, 15 set to 0x{0:X}, 0x{1:X} respectively.", MaskPriority(value >> 16), MaskPriority(value >> 24));
                break;
        ...
        private byte MaskPriority(uint value)
        {
            return (byte)(value & priorityMask);
        }
