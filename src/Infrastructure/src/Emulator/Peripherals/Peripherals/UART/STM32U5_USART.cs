...
namespace Antmicro.Renode.Peripherals.UART
{
    ...
    public sealed class STM32U5_USART : UARTBase, IUARTWithBufferState, IDoubleWordPeripheral, IKnownSize
    {
        public STM32U5_USART(IMachine machine, uint frequency, bool lowPowerMode = false) : base(machine)
        {
            IRQ = new GPIO();
            ReceiveDmaRequest = new GPIO();
            TransmitDmaRequest = new GPIO();
        ...
        public override void Reset()
        {
            ...
            TransmitDmaRequest.Unset();
            transmitDmaRequestScheduled = false;
        }
        ...
        public GPIO TransmitDmaRequest { get; }
        ...
        private void DefineRegisters()
        {
            var cr1 = Registers.ControlRegister1.Define(RegistersCollection)
                .WithFlag(0, out enabled, writeCallback: (_, __) =>
                {
                    RequestReceiveDmaIfNeeded();
                    RequestTransmitDmaIfNeeded();
                    UpdateInterrupt();
                }, name: "UE")
            ...
            var cr3 = Registers.ControlRegister3.Define(RegistersCollection)
                ...
                .WithFlag(6, out receiveDmaEnabled,
                    writeCallback: (_, __) => RequestReceiveDmaIfNeeded(),
                    name: "DMAR")
                .WithFlag(7, out transmitDmaEnabled,
                    writeCallback: (_, __) => RequestTransmitDmaIfNeeded(),
                    name: "DMAT")
            ...
            var isr = Registers.InterruptAndStatus.Define(RegistersCollection, lowPowerMode ? 0xC0u : 0x200000C0u)
                .WithFlag(0, out parityError, FieldMode.Read, name: "PE")
                .WithFlag(1, out framingError, FieldMode.Read, name: "FE")
                .WithFlag(2, out noiseDetectionFlag, FieldMode.Read, name: "NF")
                .WithFlag(3, out overrunError, FieldMode.Read, name: "ORE")
            ...
            var icr = Registers.InterruptFlagClear.Define(RegistersCollection)
                .WithFlag(0, FieldMode.Read | FieldMode.WriteOneToClear, name: "PECF",
                    writeCallback: (_, val) => { if(val) parityError.Value = false; })
                .WithFlag(1, FieldMode.Read | FieldMode.WriteOneToClear, name: "FECF",
                    writeCallback: (_, val) => { if(val) framingError.Value = false; })
                .WithFlag(2, FieldMode.Read | FieldMode.WriteOneToClear, name: "NCF",
                    writeCallback: (_, val) => { if(val) noiseDetectionFlag.Value = false; })
                .WithFlag(3, FieldMode.Read | FieldMode.WriteOneToClear, name: "ORECF",
                    writeCallback: (_, val) => { if(val) overrunError.Value = false; })
            ...
            var presc = Registers.PrescalerRegister.Define(RegistersCollection)
        ...
        private void HandleTransmitData(uint value)
        {
            if(transmitEnabled.Value && enabled.Value)
            {
                ...
                // RequestTransmitDmaIfNeeded();
                // Do not call RequestTransmitDmaIfNeeded() directly here.
                ScheduleTransmitDmaRequest();
            }
            else
            {
        ...
        private void ScheduleTransmitDmaRequest()
        {
            if(transmitDmaRequestScheduled)
            {
                return;
            }

            transmitDmaRequestScheduled = true;

            Machine.ScheduleAction(TimeInterval.FromMicroseconds(1), _ =>
            {
                transmitDmaRequestScheduled = false;
                RequestTransmitDmaIfNeeded();
            }, name: $"{nameof(STM32U5_USART)} TX DMA request");
        }
        ...
        private void RequestReceiveDmaIfNeeded()
        {
            if(receiveDmaEnabled.Value && IsReceiveEnabled && Count != 0)
            {
                ReceiveDmaRequest.Blink();
            }
        }

        private void RequestTransmitDmaIfNeeded()
        {
            // This model treats TXE as always true, so as soon as UE, TE, and DMAT
            // are enabled, USART can accept another byte into TDR.
            if(transmitDmaEnabled.Value && enabled.Value && transmitEnabled.Value)
            {
                TransmitDmaRequest.Blink();
            }
        }
        ...
        // Interrupt Flags
        private IFlagRegisterField parityError;
        private IFlagRegisterField framingError;
        private IFlagRegisterField noiseDetectionFlag;
        ...
        private enum Registers
        {
            ...
            ReceiveData        = 0x24,
            TransmitData       = 0x28,
            PrescalerRegister  = 0x2C,
            AutoModeControl    = 0x30,
        }
    }
}
