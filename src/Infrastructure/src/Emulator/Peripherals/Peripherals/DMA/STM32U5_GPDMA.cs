...
{
    public class STM32U5_GPDMA : IDoubleWordPeripheral, IKnownSize, IGPIOReceiver, INumberedGPIOOutput
    {
        public STM32U5_GPDMA(IMachine machine, int numberOfChannels)
        {
        ...
        public void OnGPIO(int requestId, bool value)
        {
            this.Log(LogLevel.Warning, "DMA request {0}: GPIO {1}", requestId, value);
            if(!value)
            {
                return;
            }
            ...
            this.Log(LogLevel.Warning, "Test");

            foreach(var channel in channels)
            {
                if(channel.Enabled && channel.MatchesRequest(requestId))
                {
                    handled |= channel.TryTriggerPeripheralTransfer(requestId);
                }
            }

            if(!handled)
            {
                this.Log(LogLevel.Noisy, "DMA request {0} ignored: no enabled matching channel", requestId);
            }
        }
        ...
        {
            public Channel(STM32U5_GPDMA parent, IGPIO interrupt, int number)
            {
                this.parent = parent;
                ...
                registersMap.Add((long)ChannelRegisters.ChannelTransfer2 + (number * ShiftBetweenChannels), new DoubleWordRegister(parent)
                    ...
                    .WithValueField(0, 7, out hardwareRequestSelection, name: "Hardware request selection (REQSEL)")
                    .WithReservedBits(7, 2)
                    .WithFlag(9, out softwareRequest, name: "Software request (SWREQ)")
                    .WithFlag(10, out destinationHardwareRequest, name: "Destination hardware request (DREQ)")
                    ...
                    .WithEnumField(16, 7, out triggerEventInputSelection, name: "Trigger event input selection (TRIGSEL)")
            ...
            {
                if(!Enabled || blockNumberDataBytesFromSource.Value == 0)
                {
                    return false;
                }
                DoTransfer();
            ...
            public bool MatchesRequest(int requestId)
            {
                return (int)hardwareRequestSelection.Value == requestId;
            }

            public bool TryTriggerPeripheralTransfer(int requestId)
            {
                parent.Log(LogLevel.Noisy, "GPDMA Try: channel={0}, requestId={1}, EN={2}, BNDT={3}, REQSEL={4}",
                    channelNumber, requestId, Enabled, blockNumberDataBytesFromSource.Value, hardwareRequestSelection.Value);
                ...
                try
                {
                    ...
                    } while(pendingPeripheralRequest);
                }
                finally
                {
            ...
            private void DoSingleBeat()
            {
                parent.Log(LogLevel.Noisy,
                    "GPDMA beat ch={0}: SA=0x{1:X}, DA=0x{2:X}, BNDT={3}, SINC={4}, DINC={5}, SDW={6}, DDW={7}",
                    channelNumber,
                    currentSourceAddress,
                    currentDestinationAddress,
                    blockNumberDataBytesFromSource.Value,
                    sourceIncrementingBurst.Value,
                    destinationIncrementingBurst.Value,
                    sourceDataWidth.Value,
                    destinationDataWith.Value);
                // BNDT is in bytes. For USART DMA, usually source/destination width is 8-bit,
                // but use the configured larger beat size defensively.
                var beatSize = Math.Max(
                    (uint)SizeToType(sourceDataWidth.Value),
                    (uint)SizeToType(destinationDataWith.Value)
                );

                var remaining = blockNumberDataBytesFromSource.Value;
                var toCopy = Math.Min(beatSize, remaining);

                var readAddress = currentSourceAddress;
                var writeAddress = currentDestinationAddress;

                // Advance state before IssueCopy, because IssueCopy can synchronously
                // write to USART TDR and cause another DMA request.
                if(sourceIncrementingBurst.Value)
                {
                    currentSourceAddress += toCopy;
                    sourceAddress.Value = currentSourceAddress;
                }

                if(destinationIncrementingBurst.Value)
                {
                    currentDestinationAddress += toCopy;
                    destinationAddress.Value = currentDestinationAddress;
                }

                blockNumberDataBytesFromSource.Value = remaining - toCopy;

                if(originalBlockNumberDataBytesFromSource != 0)
                {
                    HalfTransfer =
                        blockNumberDataBytesFromSource.Value <= originalBlockNumberDataBytesFromSource / 2;
                }

                if(blockNumberDataBytesFromSource.Value == 0)
                {
                ...
                IssueCopy(
                    ...
                    writeAddress,
                    (uint)toCopy,
                    false, // already updated source address above
                    false, // already updated destination address above
                    sourceDataWidth.Value,
                    destinationDataWith.Value
                );
            }
            ...
            private readonly IFlagRegisterField destinationHardwareRequest;
            ...
            private readonly IValueRegisterField hardwareRequestSelection;
            ...
            private readonly DoubleWordRegisterCollection registers;
            private readonly STM32U5_GPDMA parent;
